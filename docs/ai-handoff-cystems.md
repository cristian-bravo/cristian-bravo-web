...
- Home actual: las cuatro secciones originales de `src/components/home/sections/`, contenido en `src/data/es/home.ts` / `src/data/en/home.ts` y estilos `src/styles/home.css`. Logo flotante, órbitas, scanline y fondos animados originales.
- Sistema compartido: `global.css`, tipografías Manrope/Sora, layouts y Header/Footer originales. `site-polish.css` sólo añade foco/skip link y ajustes funcionales. El propietario rechazó el rediseño editorial de `ae7aa75`: no restaurarlo ni cambiar la identidad visual sin nueva solicitud.
- Proyectos actual: `src/pages/proyectos.astro` y `src/scripts/projectsScrollAnimation.ts`; escenas animadas en escritorio y scroll nativo en móvil o movimiento reducido. Las escenas ocultas son inertes y su estado se restaura al redimensionar.
- Servicios: catálogo original ES/EN y modales accesibles.
- Yuki: `src/components/YukiAssistant.astro` llama exclusivamente al proxy `/api/yuki-chat`, conectado por HTTPS a `deploy/yuki-site-gateway/`. Persona pública y modelo Ollama existente `qwen2.5:0.5b`, sin agente privado, herramientas, DB, Redis ni importación del worktree concurrente `../yuki-bot`. Token server-side, CA privada de transporte y red interna dedicada. El historial y el aprendizaje duradero siguen deshabilitados; `remember:true` falla cerrado. Véase el estado vigente de `yuki-integration.md`.
...
- Entrada comercial: las dos tarjetas originales (consulta rápida y proyecto completo) en `/empezar-proyecto`. Los enlaces guardados con `intent=new|improve|rescue` siguen validados y compatibles con el formulario simple; no modifican la composición original de la página.
